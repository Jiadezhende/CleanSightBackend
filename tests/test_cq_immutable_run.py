...
def test_cq_identity_immutable_and_reuse_machinery_gone():
    cq = ClientQueues(task_id=7, step_id=3, source_ip="ip1", stage="3")
    assert cq.task_id == 7
    assert cq.step_id == 3
    assert cq.stage == "3"
...
def test_step_id_stored_verbatim():
    ...
    cq = ClientQueues(task_id=1, step_id=None, source_ip="ip")
...
def test_client_manager_set_replaces_slot_with_new_object():
    cm = ClientManager()
    cq1 = ClientQueues(task_id=1, step_id=1, source_ip="c")
    cm.set(1, cq1)               # int 键
    assert cm.get(1) is cq1

    # 换 run = 建**新** CQ 换槽（不在旧对象上改）
    cq2 = ClientQueues(task_id=1, step_id=1, source_ip="c")
    cm.set(1, cq2)
