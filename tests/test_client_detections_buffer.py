"""CQ 的 detections 落盘缓冲（`ca_detections`）—— 与 `ca_raw` / `ca_processed` 同族的第三条。
...
def _detection_drops() -> float:
    return frame_drop_total.labels(reason="detection_backpressure")._value.get()


def test_append_then_drain_roundtrip():
    cq = make_cq(task_id=1, step_id=2, source_ip="ip")
    for ts in (1.0, 2.0, 3.0):
        cq.append_ca_detections(make_frame_detection(ts=ts))

    assert [f.ts for f in cq.drain_ca_detections()] == [1.0, 2.0, 3.0]  # 保持写入序
    assert cq.drain_ca_detections() == []                               # 取走即清，不重复交付


def test_full_buffer_drops_oldest_and_counts():
    cq = make_cq(task_id=1, step_id=2, source_ip="ip", ca_maxlen=2)
    before = _detection_drops()

    for ts in (1.0, 2.0, 3.0):
        cq.append_ca_detections(make_frame_detection(ts=ts))

    assert [f.ts for f in cq.drain_ca_detections()] == [2.0, 3.0]  # 最旧那帧被挤掉
    assert _detection_drops() - before == 1.0
...
def test_draining_run_rejects_writes():
    ...
    cq.append_ca_detections(make_frame_detection(ts=1.0))
...
def test_close_releases_the_buffer():
    ...
    assert cq.drain_ca_detections() == []
    assert cq.get_queue_depths()["ca_detections"] == 0
...
def test_slide_window_and_buffer_are_independent():
    ...
    assert [f.ts for f in cq.drain_ca_detections()] == [1.0, 2.0, 100.0]
