...
背景：detector 曾在推理时各自 time.time() 生成 DetectorOutput.timestamp，破坏"同帧多流 ts 精确
相等（来自同一 FrameInference.timestamp）"的不变式。写回口按 `FrameDetection(ts=res.timestamp,
by_source=res.detections)` 物化整帧多流；帧窗算子用 FrameDetection.ts 裁窗、用投影出的
DetectorOutput.timestamp 推进游标，两者必须同源同值，否则内部对齐错乱。

修复：帧捕获 ts 从 StageWorker.infer_batch 一路穿到 detector，令每帧
DetectorOutput.timestamp == FrameInference.timestamp == Frame.timestamp。本用例锁死该不变式。
"""
...
def test_multi_detector_same_frame_shares_capture_ts():
    """同一帧经两个 detector 后，两流 DetectorOutput.timestamp 都等于 req.timestamp。"""
    worker = StageWorker(
        stage="1",
    ...
    # 核心不变式：两流的 DetectorOutput.timestamp 相等且等于帧捕获真值锚点
...
def test_frame_feature_carries_all_streams_at_capture_ts():
    """两流经 StageWorker 后，写回口物化的 FrameDetection 携两流、ts = 帧捕获锚点（取代旧 _zip_by_ts 对齐）。"""
    worker = StageWorker(
        stage="1",
        models=[_mock_detector("streamA"), _mock_detector("streamB")],
    )
    ...
    # 写回口构造：FrameDetection(ts, by_source, frame_width, frame_height)——多流天然对齐同帧。
    feat = FrameDetection(
