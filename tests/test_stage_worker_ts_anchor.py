...
相等（来自同一 FrameDetection.ts）"的不变式。StageWorker 直接组装整帧多流的 `FrameDetection`；
帧窗算子用 FrameDetection.ts 裁窗、用投影出的
DetectorOutput.timestamp 推进游标，两者必须同源同值，否则内部对齐错乱。

修复：帧捕获 ts 从 StageWorker.infer_batch 一路穿到 detector，令每帧
DetectorOutput.timestamp == FrameDetection.ts == Frame.timestamp。本用例锁死该不变式。
"""
...
def test_multi_detector_same_frame_shares_capture_ts():
    ...
    assert fi.ts == ts
    assert set(fi.by_source) == {"streamA", "streamB"}
    # 核心不变式：两流的 DetectorOutput.timestamp 相等且等于帧捕获真值锚点
    for name, fd in fi.by_source.items():
...
def test_frame_feature_carries_all_streams_at_capture_ts():
    ...
    assert fi.ts == ts
    assert set(fi.by_source) == {"streamA", "streamB"}
    assert all(fd.timestamp == ts for fd in fi.by_source.values())
