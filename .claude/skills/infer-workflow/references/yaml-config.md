  ...
  "1":                                # 主键 = step_id（task.current_step）
    ...
    detectors:
      - name: bubble                                                          # 产出流名 = slide_window key
        class: app.services.inference.online.detection.impl.bubble.BubbleDetector    # Detector 全路径
    ...
    rules:
      - name: bubble_leak                                              # 算子自身/输出身份（≠ 流名）
        ...
        class: app.services.inference.online.temporal.impl.bubble.BubbleOperator  # Operator 全路径
...
`GRUOperator` 子类（见 templates.md 模板 D / [temporal/impl/clean.py](../../../../app/services/inference/online/temporal/impl/clean.py)）当前把 `model_path` / `objects` / `actions` / `hidden` / `num_layers` / `window_seconds` / `min_frames` 走 `rules[].params`：
...
各 `impl/__init__.py`（`detection/impl` / `temporal/impl` / `offline/impl` 各一个）是**纯包标记**，不 re-export。StageFactory 用 `class` 全路径 importlib 实例化，消费方走单文件深路径导入（`from app.services.inference.online.detection.impl.bubble import BubbleDetector`）。**别**往 `__init__.py` 加 import / `__all__`——那会让 import 本包即 eager 拉起全部任务模块。
