...
新时序分析器 = 一个 `Operator` 子类（analyze+judge 合一、单 `self._sm`）内嵌序列模型（GRU/Transformer…），把检测框滑窗喂成动作/状态。架构原理见 [DESIGN_DETECTION_WORKFLOW.md](../../../docs/kb/DESIGN_DETECTION_WORKFLOW.md) 与 [operator.py](../../../app/services/inference/online/temporal/operator.py) docstring，本 skill 只讲**怎么审**。
...
- **class_id 局部→全局映射**：`base = class_id * 4` 是否把**模型局部 id** 当**全局槽位**？上游 `class_id` 是各 YOLO 自己 `result.names` 的原始 id（[detector.py:139](../../../app/services/inference/online/detection/detector.py)）。多流共写一张特征表时，流 A 的 id=0 与流 B 的 id=0 撞槽 → 必须经 `self.objects` 显式重映射，别赌"两 .pt 共享全局标签"。
...
- **必须有显式重采样层**：入模前按帧 `ts` 把窗口重采样到模型契约帧率（见 [operator.py](../../../app/services/inference/online/temporal/operator.py) `_resample_by_ts`）；检测密度 ⟂ 模型节奏，中间隔一层，别把检测采样率焊死成模型输入率。
...
- **推理 seam**：惰性加载（双检锁）+ `model.eval()` + `torch.no_grad()`；torch/模型 import 下沉、模块顶层不引 torch（同 [detector.py](../../../app/services/inference/online/detection/detector.py) 把 ultralytics 延迟到加载处）；权重缺失显式报错。
...
- **放置**：可复用模型基类（如 `GRUOperator`）放 [operator.py](../../../app/services/inference/online/temporal/operator.py) 抽象 `Operator` 旁，与 `YOLODetector`/`Detector` 同构；具体规则放 `temporal/impl/<业务>.py`（一文件一基类）。
