...
- **`--current-step`** 决定「**跑什么**」——任务阶段，路由到对应推理 workflow：
  ...
  - `2` → CLEAN（清洁）
  - 其它未配置的数字 → MOCK（兜底透传）
  - 非数字 → 参数错误，`/api/start` 直接失败（不走 MOCK）
...
| `--current-step` | 随场景 | 任务阶段（1=LEAK / 2=CLEAN / 其它数字=MOCK）；显式指定覆盖场景默认 |
...
`--scenario 1 --current-step 99` 的预设别名：未配置的 current_step fallback 到 MOCK stage，验证帧透传不黑屏。
后端日志关键字：`未知的 step_id '99'，路由到 MOCK stage`。
...
| `--current-step` | 无 | 透传给每个子进程（`1`=LEAK / `2`=CLEAN / 其它数字=MOCK） |
| `--video_path` | `integration_tests/fixtures/test_video.mp4` | 测试视频 |
