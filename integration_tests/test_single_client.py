...
用法:
    python integration_tests/test_single_client.py --scenario <1-6> --task_id <id> [options]
...
场景说明:
  ...
  6  延迟推流:        先 start（无流，预期失败）→ N秒后推流 → 验证自动重连 (Bug 2)
    ...
    parser.add_argument("--scenario", type=int, required=True, choices=[1, 2, 3, 4, 5, 6], help="测试场景编号")
