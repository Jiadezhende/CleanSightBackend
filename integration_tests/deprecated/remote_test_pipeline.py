...
from integration_tests.utils import APIClient, FFmpegController
    ...
    def _start_rtmp_capture(self):
        ...
        try:
            ...
            self.api.start_rtmp_capture(
                client_id=self.client_id, rtmp_url=local_rtmp_url, fps=30
            )
...
def main():
    ...
    parser.add_argument(
        "--server", "-s", type=str, required=True, help="远程服务器IP地址"
    )
    parser.add_argument("--task_id", type=int, default=1, help="任务ID (默认: 1)")
    parser.add_argument(
        "--client_id", type=str, default="remote_test_client", help="客户端ID"
    )
    parser.add_argument("--duration", "-d", type=int, default=60, help="测试时长（秒）")
