...
- **改了什么**：三件事——(1) 模块/类/入口重命名收口；(2) 子进程 entrypoint `run_stages`（原 run_infer_worker）从独立文件并入 `stage_worker.py`；(3) 把「组批 + 提交」从检测服务（`ModelWorkerService`→`DetectionService`）的 N 个 per-stage 线程**合流进 dispatcher 单循环**，dispatcher 成为唯一提交者。
- **为什么改**：命名名不副实（`MultiModelWorkerPool` 无并发/复用、非线程池，且与 viz `pool.py` 撞名；`remote_infer` 冗长）；`infer_worker` 独立文件被误当「承重隔离」，核实后拆分非承重（见下）；per-stage 提交线程是 2ms 窗口/1ms sleep 的忙自旋，多线程还徒增主进程 GIL 争用——正是拆进程要治的病根。
...
| `detection/pool.py` | `detection/stage_worker.py` | 类 `MultiModelWorkerPool`→`StageWorker`；**删「pool」一词**（无并发/复用，非池，见 §2）；与 `visualization/pool.py` 区分 |
| `detection/infer_worker.py` | 并入 `stage_worker.py` 底部；`run_infer_worker`→`run_stages` | 见 §2 |
| `tests/test_remote_infer_proxy.py` | `tests/test_infer_proxy.py` | 跟随 |
| `tests/test_pool_ts_anchor.py` | `tests/test_stage_worker_ts_anchor.py` | 跟随 |

### 2. `stage_worker.py` — StageWorker 重命名 + entrypoint 并入 + 顶层 torch 惰性化

类 `MultiModelWorkerPool` → **`StageWorker`**（历史 CUDA Stream 并行已拆、现纯串行，不是池）；entrypoint `run_infer_worker` → **`run_stages`**，从独立 `infer_worker.py` 并入 `stage_worker.py`。「pool」一词从 detection 词汇表删除——子进程那个 `{stage: StageWorker}` 是启动建一次的固定 map、路由即一次 dict 查找，够不上池、不单开类；`run_stages` 建它并按 stage 路由，就是「stage 级检测服务」的子进程主函数。
...
>
> 但仍保留「顶层 torch-free」纪律：原顶层 `import torch` 改为 [`warmup`](../../app/services/inference/detection/stage_worker.py) 内惰性 import。这是**防御性**的（守住 env-先于-torch 的隔离序、与 `offline/cli.py` 对称），不是硬崩溃防护。验证：`import stage_worker` 后 `torch` 不在 `sys.modules`。
...
- [`DetectionService`](../../app/services/inference/detection/service.py)（原 `ModelWorkerService`）：proxy **先于** dispatcher 构造（要注入其 submit/capacity）；`start/stop` 去掉线程编排；删除 `_inference_loop`、`_stop_event`/`_worker_threads`、`BATCH_TIMEOUT_MS`，及随之变死的 `threading`/`time`/`guarded_run`/`AppError` import。
...
- **`StageWorker.infer_batch`（进程内组装路径）**：仍留作单测/回退。
...
| 项 | 结果 |
|----|------|
| 相关单测 | `test_infer_proxy` / `test_stage_worker_ts_anchor` / `test_pipeline_drop_counters` / `test_inference_stage_routing` 全绿 |
| 顶层 torch-free | `import stage_worker` 后 `torch` 不在 `sys.modules` |
