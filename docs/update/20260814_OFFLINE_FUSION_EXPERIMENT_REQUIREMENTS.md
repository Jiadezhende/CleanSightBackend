...
融合是**模型侧结构**，在训练仓实现；导出器的职责只是**保证各路特征逐帧对齐并声明块边界**。

| 编号 | 策略 | 结构 | 要回答的问题 | 导出侧前置 |
|------|------|------|--------------|-----------|
| **F0** | 早期 concat | `[bbox‖visual] → LayerNorm → Linear` | 视觉信息有没有用（最低成本基线） | 已就绪 |
| **F1** | 分支等宽投影 | `bbox→Linear(64)`、`visual→Linear(64)` 后融合 | **带宽失衡是不是真问题**（与 F0 直接对照） | 已就绪 |
| **F2** | 门控融合 | F1 之上，用 `visual_valid` + 联合上下文生成逐模态 gate | 模态缺失时能否自动降权 | 已就绪（mask 已导出） |
| **F3** | 手部 query 交叉注意力 | hand token 作 query 查全图/对象 token | 细粒度动作是否主要由「手在和什么交互」决定 | 需 R2 hand tokens |

**F1 vs F0 是最关键的一组对照**：它把「视觉块按维度数占走带宽、把 bbox 挤到边缘」这个担心，从只能靠代理指标猜的问题，变成一次可直接实验的对照。

原提案里的「后期融合」（两路各出逐帧 logits 再校准加权）**降级不做**：其主要价值是单路降级方案，在多路尚未证明有用之前做它是空转。

#### 维度控制：怎么避免视觉特征占走带宽

现有模型首层是 `LayerNorm(in_dim) → Linear(in_dim, hidden)`，**按维度数分配贡献**。实测（`database/99/2`，逐特征标准化后）：bbox 71 维拿 31.5%、视觉 256 维拿 68.5%，接近维度比；换 ResNet18（512 维）则变成 23.6% / 76.4%——**比例随 backbone 变，故手工调维度比不可行**（等于把超参绑死在数据格式上）。

三层手段，只有第一层是根本解法：

| 层 | 手段 | 采纳 |
|----|------|------|
| **结构层** | 各分支独立投影到**等宽**（如各 64）再融合。带宽由结构保证，与原始维度数无关，换 backbone 不受影响；顺带解决尺度失衡（每路可各自 LayerNorm） | **采纳**，即 F1/F2/F3 的共同前提 |
| 导出层 | 随机投影降维（固定 seed 的 JL 投影，不拟合统计量） | **不做**：存储非瓶颈（10 分钟 8.6MB），且结构层已从根上解决 |
| 导出层 | PCA 降维 | **禁止**：要拟合统计量，那属训练侧真源；落在导出器里就是第二份会漂移的真源（同「sidecar 不记 eff_fps」的道理） |
| 模型层 | per-branch dropout、ModDrop 模态随机失活 | 训练仓自选 |

> 需要澄清一处早期误判：拼接输出**并不损失可分性**——各块列连续、`feature_names` 完整，训练仓 slice 即可分开，F1~F3 全都能实现。真正缺的只是**显式声明块边界**，见下。

#### `manifest.blocks`：显式声明块边界

产物 manifest 增加 `blocks` 字段，声明各块的列区间 `{块名: [起, 止)}`：

```json
"blocks": {"bbox": [0, 71], "visual_global": [71, 327], "visual_valid": [327, 328]}
```

让训练仓按块切分，而不是靠 `feature_names` 前缀猜——后者在加新块时很脆。

#### F3 的 hand token：**池化成向量**

RoIAlign 结果对空间维做全局池化，每手得一个向量：

- 每手 = 浅层通道数 **128 维**（YOLO 与 ResNet18 的 stride-8 层均为 128 通道）；
- 双手 + mask，入模增 **258 维**；
- 10 分钟作业约 **9.2 MB**。

不保留空间网格（7×7 会到 452 MB/条，且在当前数据量下输入维度膨胀风险更大）。
