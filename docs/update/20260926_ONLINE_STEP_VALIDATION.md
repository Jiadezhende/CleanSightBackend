...
- **降级帧不落盘**：运行时推理失败仍逐帧降级、画面照常，但降级帧不再写进 `detections.jsonl`。

影响范围：`RunController.start_run`、`InferenceManager.resolve_stage` / `_get_stage_configs`、`StageFactory`、检测写回（`DetectionService._write_back_results`）、YAML，以及集成测试场景 8。
...
- **现状**：`resolve_stage` 把所有没进入生效集合的 step 都路由到 MOCK 透传，不区分「YAML 里没配」和「配了但 detector 构造失败」。
  ...
  - 降级帧（`success=False`、`boxes=[]`）照常进落盘缓冲，而落盘格式不带 `success`，`detections.jsonl` 里「推理失败」和「这帧确实没检出」写成同样的空框，离线会在失败帧上跑出 `completed`。
- **定性**：
...
运行时推理异常 → 逐帧降级（success=False、boxes=[]）
   帧窗 / 最新检测结果照写（画面照常）；落盘缓冲跳过降级帧
```
...
| 降级帧不入落盘缓冲 | [`DetectionService._write_back_results`](../../app/services/inference/online/detection/service.py) |
...
| 运行时推理失败：画面 | 逐帧降级，画面照常 | 不变 |
| 运行时推理失败：`detections.jsonl` | 写入空框，离线当成「没检出」 | 不写，离线侧是时间空洞；整个 run 全失败则「无检测结果」→ skipped |
...
| `test_writeback_handle_fence.py`：降级帧进帧窗 / 快照、不进落盘缓冲 | 通过（改动前会失败） |
...
| 一个 run 一帧检测结果都没落盘（全程推理失败）时，不会清掉该 step 上一次 run 的检测结果和离线结果；录像却已换成这次的 | 回放时可能把上一次 run 的分析结果对到这次的录像上 | 暂按现状接受（recording 两个域各自「首写自清」的既定设计）；要修需让录像首写连带清检测域，动 recording 队列不变式 |
