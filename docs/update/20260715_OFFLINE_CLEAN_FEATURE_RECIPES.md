...
本次同步修改了 `tests/test_offline_pipeline.py`：

| 测试点 | 改动 |
|---|---|
| `TestCleanSegmenter.test_flatten_preprocess_to_segments` | 将旧的 `feature_dim == 68` 断言更新为基础 v2 的 `feature_dim == 113`，并补充 `feature_version == clean_bbox_v2_top1_impute` |
| `TestCleanSegmenter.test_each_clean_model_uses_own_feature_recipe` | 新增三模型 recipe 测试，分别覆盖 MS-TCN 113 维、ASFormer 121 维、BiGRU 249 维，以及各自的 `feature_version` |

这部分测试不是新增工程能力，而是把离线测试预期同步到新的模型输入契约，避免测试仍停留在旧 68 维特征。
