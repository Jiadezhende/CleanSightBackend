...
本文是**验收标准**：什么样的内容才够格写进知识库。
知识库不随日常开发更新——增量先落 `docs/update/`，由人主动发起批量融合；融合**怎么执行**（划范围、成链、复核代码、判归属、收尾自检）见 `/kb-merge` skill。
...
- 判据：**这次改动之后，跑起来的东西有没有变？** 没变 → 不落记录。
- 文档改动本身要留的东西就写在被改的文档里：搬迁后在原处留一行指向新位置；删掉过时内容
  时，若结论仍有效就写进接收方，只是过时了就直接删。
- 边界情形：文档改动**顺带**改了代码行为（改了默认值、删了函数），按代码改动处理，落记录。
- 已经是 KB 条目的内容不进「待沉淀」清单：它已经沉淀完了。
