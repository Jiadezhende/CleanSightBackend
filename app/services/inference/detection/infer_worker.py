...
进程边界只过纯数据（均 picklable）：
  ...
  resp (child→main):  (req_id:int, merged:List[Dict[str,FrameDetections]], stats:List[ModelStat])
...
def run_infer_worker(req_q, resp_q, ready_ev, active_stages: List[str], cuda_device: str = "0") -> None:
    ...
    Args:
        ...
        resp_q: 响应队列（child→main），元素 (req_id, merged, stats)。
    ...
    logger.info("推理子进程启动 pid=%s stages=%s cuda=%r", os.getpid(), active_stages, cuda_device)
    ...
    for stage in active_stages:
        ...
        if detectors:
            ...
            logger.warning("[infer-child] stage %s 无 detector，跳过", stage)
    ...
    for stage, pool in pools.items():
        try:
            ...
            logger.error("[infer-child] stage %s warmup 失败: %s", stage, e, exc_info=True)

    ready_ev.set()  # 就绪屏障：pool 建好 + warmup 尝试完成
    logger.info("[infer-child] ready，进入推理循环")
    ...
    while True:
        try:
            ...
            # 父进程关闭队列/退出 → 收摊
            logger.info("[infer-child] req_q 关闭，退出")
            break

        if item is None:
            logger.info("[infer-child] 收到退出哨兵，退出")
            break
        ...
        try:
            merged, stats = pool._infer_models(frames, timestamps)
            resp_q.put((req_id, merged, stats))
        except Exception as e:  # pragma: no cover - _infer_models 内部逐模型兜底，此为双保险
            logger.error("[infer-child] req_id=%s 推理异常: %s", req_id, e, exc_info=True)
            # 仍须应答：回 n 个空帧结果 + 一条错误 stat，主进程据此 pop pending 并计失败
            resp_q.put((
                req_id,
                [{} for _ in frames],
                [("_worker", 0.0, len(frames), type(e).__name__)],
            ))
