...
**EXTINF 是段时长的唯一真值**，不能用文件名 ts 差重新推导 —— 那是 wall-clock 抖动值，与
fMP4 fragment 的实际媒体时长不一致，会引入 hls.js 段尾停摆 / 总时长缩水。

**包内私有**，函数都收「已定位的具体路径」：服务层不知道 playlist 文件叫什么、在哪个目录，
也不该知道。对外入口是 `Step` 的成员（`segments()` / `has_init()` / `time_bounds_us` /
`vod_playlist()`）。**对外只出 m3u8 成品、不出骨架** —— 骨架写错播放器立刻报错，而备料
（EXTINF 真值、滤在途、判 init、算 TARGETDURATION）写错是静默的。

包外只有两条**具名例外**：写侧 `persistence/strategies/hls_strategy`（格式的定义者，永久）
与 `lab/clip_builder`（EXTINF 口径不同，见 `build_vod_playlist`，验证等价后撤销）。名单与
退出条件由 tests/test_import_hygiene.py 锁死，别绕过它加第三条。
...
def parse_playlist_entries(playlist_path: Path) -> List[Tuple[str, float]]:
    """解析写侧 LIVE m3u8，按出现顺序产出 [(filename, 时长秒)]。

    格式约定：每个段对应一行 `#EXTINF:<dur>,` 紧接一行 `<filename>`。解析失败或文件不存在
    返回空列表。

    **保序**是有意的：写侧 tfdt 偏移要的是「当前段以前所有 EXTINF 的前缀和」，顺序即语义；
    读侧要的是 filename → dur 映射（`parse_playlist_durations`）。一份解析供两种消费。
    """
...
def parse_playlist_durations(playlist_path: Path) -> Dict[str, float]:
    """filename → EXTINF 时长映射。解析失败或文件不存在返回空字典。

    返回值同时承担两个职责：值是段时长的唯一真值；键集合即「已完成 transcode+append 的
    段」，不在其中的是在途段、须过滤。
    """
...
def sum_durations(playlist_path: Path) -> float:
    ...
    写侧算 fMP4 fragment 的 tfdt 起点用：`tfdt(N) = Σ EXTINF(0..N-1)`。调用发生在把当前段
    append 进 playlist **之前**，故此刻求和即得本段起点。

    ⚠ 不要回退到「文件名 ts_us 差」的算法 —— 那是 wall-clock 抖动值，与 fragment 媒体时长
    不一致，会重新引入 hls.js 段尾停摆 / 总时长缩水。
    """
...
def build_vod_playlist(
    ...
    """构造 VOD m3u8 文本（含 ENDLIST）。纯字符串拼装、不碰磁盘。

    **骨架，非成品** —— 对外入口是 `Step.vod_playlist()`，它在这之上补齐全部备料。
    **只共用骨架，不统一语义**：`entries` 的 URI 形态（token 化 URL / 同目录 basename）、
    时长来源（playlist EXTINF / 相邻段 ts 跨度）、`target_duration` 的取整方式都由调用方自
    己备料，这些是真差异而非重复。
