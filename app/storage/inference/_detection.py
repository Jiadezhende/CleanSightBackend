...
def _feature_to_record(feature: FrameDetection) -> Dict[str, Any]:
    """FrameDetection → 磁盘 record（逆运算 `_record_to_feature`）。"""
    record: Dict[str, Any] = {
        "ts": feature.ts,
        "features": {
...
def _record_to_feature(rec: Mapping[str, Any]) -> FrameDetection:
...
def append_features(task_id: int, step_id: int, features: Sequence[FrameDetection]) -> None:
...
def read_features(task_id: int, step_id: int) -> List[FrameDetection]:
