"""本域的布局：域根目录、三份产物的文件名，以及整域删除。
    ...
    FEATURES_NAME / FACTS_NAME / DEBUG_NAME
...
# 产物文件名。
FEATURES_NAME = "features.jsonl"  # 每帧一行，L1 检测特征
...
def delete(task_id: int, step_id: int) -> bool:
    ...
    **三份产物一起没**：features / facts / 调试产物同去同归。同 (task, step) 重启一次 run 之
    后，旧 facts 是对旧 features 的分析结果，留着即脏数据。
