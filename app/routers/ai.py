...
async def websocket_video_endpoint(websocket: WebSocket):
    ...
    logger.info(f"[WebSocket] 连接已建立: client_id={client_id}, remote={websocket.client}")
        ...
        while True:
            ...
            try:
                ...
                if current_time - last_log_time >= 5.0:
                    ...
                    logger.debug(f"[WebSocket] client={client_id}: 发送 {frames_sent}帧/{elapsed:.1f}秒 = {fps:.1f}fps")
                    frames_sent = 0
                ...
                # 其他未预期的发送错误
                logger.error(f"[WebSocket] 发送异常: client_id={client_id}", exc_info=True)
                break
